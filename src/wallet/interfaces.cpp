...
WalletTx MakeWalletTx(CWallet& wallet, const CWalletTx& wtx)
{
    ...
    if (auto tx_state = std::get_if<TxStateConfirmed>(&wtx.m_state)) {
        result.block_hash = tx_state->confirmed_block_hash;
    }
    return result;
}
...
public:
    ...
    {
        LOCK(m_wallet->cs_wallet);
        vCoins = AvailableCoins(*m_wallet, coinControl, std::nullopt, nMinimumAmount).All();
    }
        ...
        CAmount* required_fee) override
    {
        ...
        auto res = CreateTransaction(*m_wallet, recipients, change_pos,
                                     coin_control, sign, omni, required_fee);
    ...
    std::vector<WalletTx> getWalletTxsDetails(std::map<uint256, WalletTxStatus>& tx_status) override
    {
        ...
        for (const auto& entry : m_wallet->mapWallet) {
            const auto& wtx = entry.second;
            tx_status.emplace(wtx.GetHash(), MakeWalletTxStatus(*m_wallet, wtx));
            result.emplace_back(MakeWalletTx(*m_wallet, wtx));
        }
        return result;
    }
